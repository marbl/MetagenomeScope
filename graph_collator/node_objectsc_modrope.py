...
class Node(object):
    ...
    def __init__(self, id_value, bp, is_complement, use_rc_prefix=True):
        ...
        # This should be False for every basic node and for most node
        # groups. It's only used when we incorporate an already-created
        # node group (e.g. a chain) into another node group, "deleting"
        # the original chain.
        self.is_deleted_group = False
...
class Bubble(Node):
    ...
    @staticmethod
    def is_valid_bubble(s, middle_nodes):
        e = None
...
class Rope(Node):
    ...
    # Returns a 2-tuple of (True, a tuple of all the nodes in the Rope
    # if a Rope defined with the given node as one of its start nodes
    # would be a valid Rope. Returns a 2-tuple of (False, None) if such
    # a Rope would be invalid.
    # This assumes that the passed node has exactly one outgoing node.
    @staticmethod
    def is_valid_rope(s):
        #to_delete_chain = False
        ## Determine first "middle" node
        #rope_middle_start = s.outgoing_nodes[0]
        ## Determine all "start" nodes
        #starting_nodes = rope_middle_start.incoming_nodes
        #if len(starting_nodes) < 2: return False, None
        ## Determine all "middle" nodes
        #chain_validity, middle_nodes = Chain.is_valid_chain(rope_middle_start)
        ...
        #    if (rope_middle_start.seen_in_collapsing and
        #            type(rope_middle_start.group) == Chain):
        #        to_delete_chain = True
        #        middle_nodes = rope_middle_start.group.nodes
        #        rope_end = middle_nodes[len(middle_nodes) - 1].outgoing_nodes
        #    else:
        #        # Or we could just have a single middle node
        #        rope_end = rope_middle_start.outgoing_nodes
        #        middle_nodes = [rope_middle_start]
        #else:
        #    # The middle nodes form a chain, but a chain including the
        #    # middle nodes does not exist yet
        #    rope_end = middle_nodes[len(middle_nodes) - 1].outgoing_nodes
        ## NOTE following is original is_valid_rope

        # If the rope ends without diverging to >= 2 end nodes (e.g.
        # there's an extra incoming edge that messes things up)
        ...
        # be valid.
        #if to_delete_chain: rope_middle_start.group.is_deleted_group = True
