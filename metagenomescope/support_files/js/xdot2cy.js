...
function rotateNode(n, i) {
    ...
    var newPt = util.rotateCoordinate(oldPt.x, oldPt.y);
...
function renderNodeObject(nodeObj, cyNodeID, boundingboxObject, mode) {
    ...
    var gcColor = null;
    if (nodeGC !== null) {
        gcColor = util.getNodeColorization(nodeGC);
    }
    ...
    if (nodeIsRepeat !== undefined) {
        if (nodeIsRepeat === null) {
            ...
            // We could call util.getNodeColorization() with nodeIsRepeat, but
...
function renderClusterObject(clusterObj, boundingboxObject, spqrtype) {
    ...
    var bottomLeftPos = util.gv2cyPoint(clusterObj[l], clusterObj[b], [
        boundingboxObject.boundingbox_x,
        boundingboxObject.boundingbox_y,
    ]);
    var topRightPos = util.gv2cyPoint(clusterObj[r], clusterObj[t], [
        boundingboxObject.boundingbox_x,
        boundingboxObject.boundingbox_y,
    ]);
    var clusterData = {
...
function renderEdgeObject(
    ...
    var srcSinkDist = util.distance(srcPos, tgtPos);
    var ctrlPts = util.ctrlPtStrToList(edgeObj.control_point_string, [
        boundingboxObject.boundingbox_x,
        boundingboxObject.boundingbox_y,
    ]);
    ...
    for (var p = 0; p < ctrlPtLen; p++) {
        currPt = ctrlPts[p];
        pld = util.pointToLineDistance(currPt, srcPos, tgtPos);
        pldsquared = Math.pow(pld, 2);
        dsp = util.distance(currPt, srcPos);
        dtp = util.distance(currPt, tgtPos);
