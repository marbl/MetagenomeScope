...
def test_component_sorting_ecoli_graph():
    ...
    for node_id in wccs[0][0]:
        if not ag.is_pattern(node_id):
            if ag.nodeid2obj[node_id].name == "89":
                seen_89_in_largest_cc = True
                break
        else:
            # Recursively go through pattern to check for node 89
            # Why did I write this test in this way, and not just check for a
            # node that we know will always be in the top level? Mmmm I have a
            # really good answer for that, but I leave it as an exercise to the
            # reader so yeah sorry can't explain it here uh bye my bad
            pattern_queue = [node_id]
            while len(pattern_queue) > 0:
                patt_id = pattern_queue.pop()
                patt_obj = ag.pattid2obj[patt_id]
                for child_node in patt_obj.nodes:
                    child_node_id = child_node.unique_id
                    if ag.is_pattern(child_node_id):
                        pattern_queue.append(child_node_id)
                    else:
                        if ag.nodeid2obj[child_node_id].name == "89":
                            seen_89_in_largest_cc = True
                            break
