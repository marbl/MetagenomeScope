...
1. `collate.py`, a Python script that reads an assembly graph file,
   ...
   (Bambus 3 scaffold), and GFA assembly graph files, and support for GFA2
   and FASTG (SPAdes) files is planned. Note that, although GFA files only have
   DNA sequences as an optional requirement, including DNA sequences is highly
   recommended: this facilitates, among other useful features,
   visual node scaling based on DNA length. Nodes that lack a DNA sequence in
   GFA files are currently just assigned a length of "100 bp"
