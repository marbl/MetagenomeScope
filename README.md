...
1. The **preprocessing script** (contained in the `metagenomescope/` directory of
   ...
   - **Once you've installed the preprocessing script on your system, you
     can run this script using the command `mgsc`.**
   - If the `-spqr` option is passed to `mgsc`, it uses the C++ code in
     `spqr.cpp` to interface
     with [OGDF](https://ogdf.uos.de/) to generate SPQR tree decompositions of
     biconnected components in the graph for MetagenomeScope's "decomposition
     mode." Since this requires some C++ code to be compiled for your system,
     the use of `-spqr` in MetagenomeScope necessitates a few extra
     installation steps. See
     [the installation instructions page](https://github.com/marbl/MetagenomeScope/wiki/Installation-Instructions)
...
2. The **viewer interface** (contained in the `viewer/` directory of this
   ...
   generated by `mgsc` and renders the resulting graph using
...
- You can also use the "Upload .db file" button to view .db files generated by
  `mgsc`. However, to ensure compatibility between your .db files and the
  viewer interface, it's probably best to use the version of the viewer
  interface bundled in the `viewer/` directory of this repository.
